...
int main(int argc, char **argv)
{
    ...
    */
    ...
    encoded_sequence = core_memory_allocate(encoded_length, -1);

    biosal_dna_codec_encode(&codec, sequence_length, dna, encoded_sequence);

    sequence2 = core_memory_allocate(sequence_length + 1, -1);
    expected_sequence = core_memory_allocate(sequence_length + 1, -1);
    ...
    */
    ...
    core_memory_free(encoded_sequence, -1);
    core_memory_free(sequence2, -1);
    core_memory_free(expected_sequence, -1);
...

    {
        ...
        encoded_data = core_memory_allocate(encoded_length, -1);
        encoded_data2 = core_memory_allocate(encoded_length, -1);
        ...
        core_memory_free(encoded_data, -1);
        core_memory_free(encoded_data2, -1);
