...
cat $(ls *gcov | grep hot_code)|grep -v '\-'|grep -v '#'|sed 's/://g'|sort -r -n | grep biosal_|grep -v ';'|grep -v ' if ' > hot_code_temperature_functions.txt
cat $(ls *gcov | grep -v hot_code)|grep -v '\-'|grep -v '#'|sed 's/://g'|sort -r -n  | grep biosal_|grep -v ';'|grep -v ' if '> code_temperature_functions.txt
