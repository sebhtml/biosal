...
void thorium_worker_init(struct thorium_worker *worker, int name, struct thorium_node *node)
{
    ...
    */
    ...
    CORE_BITMAP_CLEAR_FLAGS(worker->flags);
    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_DEAD);
    ...
    core_queue_init(&worker->input_inbound_message_queue, sizeof(struct thorium_message));
    ...
    core_queue_init(&worker->output_outbound_message_queue_for_multiplexer, sizeof(struct thorium_message));
    ...
    core_queue_init(&worker->output_message_queue_for_triage,
                    sizeof(struct thorium_message));
#endif
    ...
    if (thorium_node_must_print_data(worker->node)) {
    ...
    */
    ...
    core_queue_init(&worker->output_outbound_message_queue, sizeof(struct thorium_message));

    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_DEBUG);
    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_BUSY);
    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_ENABLE_ACTOR_LOAD_PROFILER);

    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_DEBUG_ACTORS);
...
#endif
            CORE_BITMAP_SET_FLAG(worker->flags, FLAG_DEBUG_ACTORS);
        }
    }
 ...
 */
    ...
    CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_USE_MULTIPLEXER);
...
void thorium_worker_destroy(struct thorium_worker *worker)
{
    ...
    */
    ...
    if (CORE_BITMAP_GET_FLAG(worker->flags, FLAG_ENABLE_ACTOR_LOAD_PROFILER)) {
    ...
    */
    ...
    printf("THORIUM-> output_message_queue_for_triage has %d items\n",
                    core_queue_size(&worker->output_message_queue_for_triage));
    ...
    core_queue_destroy(&worker->output_outbound_message_queue_for_multiplexer);

    core_queue_destroy(&worker->input_inbound_message_queue);
    ...
    */
    ...
    core_queue_destroy(&worker->output_message_queue_for_triage);
#endif
...
void thorium_worker_stop(struct thorium_worker *worker)
{
     ...
     */
    if (CORE_BITMAP_GET_FLAG(worker->flags, FLAG_ENABLE_WAIT)) {
        /*
...
{
    return CORE_BITMAP_GET_FLAG(worker->flags, FLAG_BUSY);
}
...
int thorium_worker_enqueue_inbound_message(struct thorium_worker *worker, struct thorium_message *message)
{
     ...
     */
    if (value && CORE_BITMAP_GET_FLAG(worker->flags, FLAG_ENABLE_WAIT)) {
...
int thorium_worker_enqueue_message(struct thorium_worker *worker, struct thorium_message *message)
{
    ...
    if (!thorium_worker_publish_message(worker, message)) {
        ...
        printf("ENQUEUE thorium_worker: Warning: ring is full (ring: %d queue: %d), output_outbound_message_ring\n",
                        ...
                        core_queue_size(&worker->output_outbound_message_queue));
#endif
        /* If that does not work, push the message in the queue buffer.
         */
        core_queue_enqueue(&worker->output_outbound_message_queue, message);
...
void thorium_worker_evict_actor(struct thorium_worker *worker, int actor_name)
{
    struct thorium_actor *actor;
    int name;
    struct core_queue saved_actors;
    ...
    */
    ...
    core_queue_init(&saved_actors, sizeof(struct thorium_actor *));
    ...
    while (thorium_scheduler_dequeue(&worker->scheduler, &actor)) {
        ...
        if (name != actor_name) {
            ...
            core_queue_enqueue(&saved_actors,
                            &actor);
        }
    }

    while (core_queue_dequeue(&saved_actors, &actor)) {
        thorium_scheduler_enqueue(&worker->scheduler, actor);
    }

    core_queue_destroy(&saved_actors);
...
static int thorium_worker_enqueue_message_for_triage(struct thorium_worker *worker, struct thorium_message *message)
{
    ...
    if (!core_fast_ring_push_from_producer(worker->output_message_ring_for_triage, message)) {
        ...
        core_queue_enqueue(&worker->output_message_queue_for_triage, message);
...
int thorium_worker_get_message_production_score(struct thorium_worker *worker)
{
    ...
    score += core_queue_size(&worker->output_outbound_message_queue);
...
void thorium_worker_run(struct thorium_worker *worker)
{
    ...
    if (elapsed >= period) {
        ...
        */
        ...
        if (CORE_BITMAP_GET_FLAG(worker->flags, FLAG_DEBUG_ACTORS)) {
     ...
     */
    if (!CORE_BITMAP_GET_FLAG(worker->flags, FLAG_OUTPUT_OUTBOUND_MESSAGE_RING_IS_FULL)
        ...
        CORE_BITMAP_SET_FLAG(worker->flags, FLAG_BUSY);
        ...
        CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_BUSY);
        CORE_BITMAP_CLEAR_FLAG(worker->flags, FLAG_ENABLE_WAIT);
     ...
     */
    if (core_queue_dequeue(&worker->output_outbound_message_queue, &other_message)) {
     ...
     */
    if (core_queue_dequeue(&worker->output_message_queue_for_triage, &other_message)) {
     ...
     */
    if (core_queue_dequeue(&worker->input_inbound_message_queue, &other_message)) {
    ...
    */
     ...
     * by another worker.
     */
    if (core_queue_dequeue(&worker->output_outbound_message_queue_for_multiplexer,
        &other_message)) {
...
static void thorium_worker_work(struct thorium_worker *worker, struct thorium_actor *actor)
{
    ...
    if (dead) {
        ...
        if (CORE_BITMAP_GET_FLAG(worker->flags,
...
{
    CORE_BITMAP_SET_FLAG(self->flags, FLAG_ENABLE_WAIT);
}
...
void thorium_worker_examine(struct thorium_worker *self)
{
    ...
    printf("QUEUE= output_outbound_message_queue size= %d\n",
                    core_queue_size(&self->output_outbound_message_queue));
    ...
    printf("QUEUE= output_message_queue_for_triage size= %d\n",
                    core_queue_size(&self->output_message_queue_for_triage));
}
...
#if 0
    if (CORE_BITMAP_GET_FLAG(node->flags, FLAG_ENABLE_ACTOR_LOAD_PROFILER)) {
...
void thorium_worker_enable_profiler(struct thorium_worker *self)
{
    ...
    CORE_BITMAP_SET_FLAG(self->flags, FLAG_ENABLE_ACTOR_LOAD_PROFILER);
...
static void thorium_worker_configure_backoff(struct thorium_worker *self)
{
    ...
    CORE_BITMAP_CLEAR_FLAG(self->flags, FLAG_OUTPUT_OUTBOUND_MESSAGE_RING_IS_FULL);
#endif
}
...
static void thorium_worker_activate_backoff(struct thorium_worker *self)
{
     ...
     */
    if (!CORE_BITMAP_GET_FLAG(self->flags, FLAG_OUTPUT_OUTBOUND_MESSAGE_RING_IS_FULL)) {

        CORE_BITMAP_SET_FLAG(self->flags, FLAG_OUTPUT_OUTBOUND_MESSAGE_RING_IS_FULL);
...
static void thorium_worker_do_backoff(struct thorium_worker *self)
{
    ...
    if (CORE_BITMAP_GET_FLAG(self->flags, FLAG_OUTPUT_OUTBOUND_MESSAGE_RING_IS_FULL)) {
         ...
         * message queue.
         */
        size = core_queue_size(&self->output_outbound_message_queue);
        capacity = 512;
#else
...
void thorium_worker_send_local_delivery(struct thorium_worker *self, struct thorium_message *message)
{
    ...
    if (!message_was_pushed)
        core_queue_enqueue(&self->output_outbound_message_queue,
                    message);
}
