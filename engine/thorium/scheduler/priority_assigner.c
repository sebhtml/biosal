...
{
    scheduler->name = name;
    core_map_init(&scheduler->actor_sources, sizeof(int), sizeof(int));
    core_map_init(&scheduler->actor_source_frequencies, sizeof(int), sizeof(int));
...
void thorium_priority_assigner_destroy(struct thorium_priority_assigner *scheduler)
{
    ...
    core_map_destroy(&scheduler->actor_sources);
    core_map_destroy(&scheduler->actor_source_frequencies);
...
void thorium_priority_assigner_update(struct thorium_priority_assigner *scheduler, struct thorium_actor *actor)
{
    ...
    */
     ...
     */
    if (core_map_get_value(&scheduler->actor_sources, &name, &old_source_count)) {
        ...
        if (new_source_count != old_source_count) {
             ...
             */
            core_map_update_value(&scheduler->actor_sources, &name, &new_source_count);
         ...
         */
        core_map_add_value(&scheduler->actor_sources, &name, &new_source_count);
...
void thorium_priority_assigner_update_thresholds(struct thorium_priority_assigner *scheduler)
{
    ...
    core_map_iterator_init(&iterator, &scheduler->actor_source_frequencies);
    ...
    class_count = 0;
    while (core_map_iterator_get_next_key_and_value(&iterator, &value, &frequency)) {
    ...
    scheduler->normal_priority_minimum_value = core_statistics_get_percentile_int_map(&scheduler->actor_source_frequencies,
                    30);
    scheduler->normal_priority_maximum_value = core_statistics_get_percentile_int_map(&scheduler->actor_source_frequencies,
                    70);
    scheduler->max_priority_minimum_value = core_statistics_get_percentile_int_map(&scheduler->actor_source_frequencies,
                    95);
                ...
                int old_source_count)
{
    ...
    if (core_map_get_value(&scheduler->actor_source_frequencies, &old_source_count,
                    &old_source_count_old_frequency)) {

        old_source_count_new_frequency = old_source_count_old_frequency - 1;

        core_map_update_value(&scheduler->actor_source_frequencies, &old_source_count,
                ...
                int new_source_count)
{
    ...
    if (core_map_get_value(&scheduler->actor_source_frequencies, &new_source_count,
                            &new_source_count_old_frequency)) {

        new_source_count_new_frequency = new_source_count_old_frequency + 1;
        core_map_update_value(&scheduler->actor_source_frequencies, &new_source_count,
        ...
        core_map_add_value(&scheduler->actor_source_frequencies, &new_source_count,
                        &new_source_count_new_frequency);
    }
}
